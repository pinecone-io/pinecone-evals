...
def main():
    ...
    def basic_search(query: Query) -> SearchResult:
        ...
        if "machine learning" in query.text.lower():
            hits = [
                SearchHit(id="ml1", text="Machine learning is a branch of artificial intelligence focused on building systems that learn from data."),
                SearchHit(id="ml2", text="Deep learning is a subset of machine learning that uses neural networks with many layers."),
                SearchHit(id="ml3", text="Supervised learning requires labeled training data.")
            ]
        elif "database" in query.text.lower():
            hits = [
                SearchHit(id="db1", text="Databases store and organize data for easy retrieval and management."),
                SearchHit(id="db2", text="SQL is a language used to query relational databases."),
                SearchHit(id="db3", text="NoSQL databases provide flexible schema design for unstructured data.")
            ]
        else:
            hits = [
                SearchHit(id="prog1", text="Python is a popular language for data science and machine learning."),
                SearchHit(id="prog2", text="JavaScript is used for web development on both client and server sides."),
                SearchHit(id="prog3", text="Rust provides memory safety without garbage collection.")
            ]
