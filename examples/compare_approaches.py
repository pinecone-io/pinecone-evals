...
def main():
    ...
    def keyword_search(query: Query) -> SearchResult:
        ...
        if "vector" in query.text.lower():
            hits = [
                SearchHit(id="vs1", text="Vector search uses vector representations to find similar items."),
                SearchHit(id="vs2", text="Traditional search uses keywords, while vector search uses embeddings."),
                SearchHit(id="vs3", text="Indexing is a crucial part of efficient vector search algorithms.")
            ]
        elif "semantic" in query.text.lower():
            hits = [
                SearchHit(id="ss1", text="Semantic search understands the intent behind search queries."),
                SearchHit(id="ss2", text="Vector databases are optimized for semantic search operations."),
                SearchHit(id="ss3", text="Embeddings capture the semantic meaning of text or images.")
            ]
        else:
            hits = [
                SearchHit(id="perf1", text="Query optimization techniques include caching and indexing."),
                SearchHit(id="perf2", text="Performance can be improved by using specialized vector indexes."),
                SearchHit(id="perf3", text="Benchmarking is essential for measuring query performance.")
            ]
    ...
    def hybrid_search(query: Query) -> SearchResult:
        ...
        if "vector" in query.text.lower() or "semantic" in query.text.lower():
            additional_hits = [
                SearchHit(id="h1", text="Hybrid search combines keyword matching with semantic understanding."),
                SearchHit(id="h2", text="Embedding models transform queries into high-dimensional vectors.")
            ]
        else:
            additional_hits = [
                SearchHit(id="h3", text="Query planning is important for complex search operations."),
                SearchHit(id="h4", text="Vector similarity metrics include cosine similarity and dot product.")
            ]
        ...
        reranked_hits = sorted(
            all_hits,
            key=lambda hit: sum(w in hit.text.lower() for w in query.text.lower().split()),
            reverse=True
        )
